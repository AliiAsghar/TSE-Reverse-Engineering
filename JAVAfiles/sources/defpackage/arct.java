            ...
            if (!e.isEmpty() && e.charAt(e.length() - 1) == 's') {
                ...
                if (parseLong >= 0) {
                    ...
                    try {
                        long j2 = a;
                        if (j <= (-j2) || j >= j2) {
                            parseLong = albo.cK(parseLong, j / j2);
                            bh = (int) (j % j2);
                        }
                        if (parseLong > 0 && bh < 0) {
                            parseLong--;
                        ...
                        if (parseLong >= -315576000000L && parseLong <= 315576000000L) {
                            long j3 = bh;
                            if (j3 >= -999999999 && j3 < j2 && ((parseLong >= 0 && bh >= 0) || (parseLong <= 0 && bh <= 0))) {
                        ...
                        throw new IllegalArgumentException(String.format("Duration is not valid. See proto definition for valid values. Seconds (%s) must be in range [-315,576,000,000, +315,576,000,000]. Nanos (%s) must be in range [-999,999,999, +999,999,999]. Nanos must have the same sign as seconds", Long.valueOf(parseLong), Integer.valueOf(bh)));
