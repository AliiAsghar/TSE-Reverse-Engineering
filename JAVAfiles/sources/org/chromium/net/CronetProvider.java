...
public abstract class CronetProvider {
    ...
    private static boolean addCronetProviderFromResourceFile(Context context, atei ateiVar, Set<atdv> set) {
        ...
        if (string != null && !string.equals(PLAY_SERVICES_CRONET_PROVIDER_CLASS) && !string.equals(GMS_CORE_CRONET_PROVIDER_CLASS) && !string.equals(JAVA_CRONET_PROVIDER_CLASS) && !string.equals(NATIVE_CRONET_PROVIDER_CLASS)) {
            z = true;
            if (!addCronetProviderImplByClassName(context, string, ateiVar, set, true)) {
    ...
    private static boolean addCronetProviderImplByClassName(Context context, String str, atei ateiVar, Set<atdv> set, boolean z) {
        try {
            ...
            atdvVar.a = (CronetProvider) constructor.newInstance(context);
            atdvVar.b = ateiVar;
            set.add(atdvVar);
            return true;
    ...
    public static List<atdv> getAllProviderInfos(Context context) {
        LinkedHashSet linkedHashSet = new LinkedHashSet();
        addCronetProviderFromResourceFile(context, atei.CRONET_SOURCE_UNSPECIFIED, linkedHashSet);
        addCronetProviderImplByClassName(context, PLAY_SERVICES_CRONET_PROVIDER_CLASS, atei.CRONET_SOURCE_PLAY_SERVICES, linkedHashSet, false);
        addCronetProviderImplByClassName(context, GMS_CORE_CRONET_PROVIDER_CLASS, atei.CRONET_SOURCE_PLAY_SERVICES, linkedHashSet, false);
        addCronetProviderImplByClassName(context, NATIVE_CRONET_PROVIDER_CLASS, atei.CRONET_SOURCE_STATICALLY_LINKED, linkedHashSet, false);
        addCronetProviderImplByClassName(context, JAVA_CRONET_PROVIDER_CLASS, atei.CRONET_SOURCE_FALLBACK, linkedHashSet, false);
    ...
    public static List<CronetProvider> getAllProviders(Context context) {
        ...
        Iterator<atdv> it = getAllProviderInfos(context).iterator();
        while (it.hasNext()) {
    ...
    private static void logReflectiveOperationException(String str, boolean z, Exception exc) {
        if (z) {
            ...
            if (Log.isLoggable(str2, 3)) {
                Log.d(str2, a.bW(str, "Tried to load ", " provider class but it wasn't included in the app classpath"));
            }
        }
    }
